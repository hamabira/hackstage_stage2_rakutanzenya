...
## 科目数
- まず入力に何科目分の情報が含まれるかを数え、detectedSubjectCount に入れる。
- 科目名が複数あり、それぞれに独立した授業回数や評価構成が書かれている場合は、必ず2以上にする。
- 1つの科目の中の複数の評価項目（中間・期末など）は、科目数ではなく評価項目として数える。
- 複数科目があっても分割や統合はせず、最初の1科目だけを抽出したうえで、正しい科目数を報告する。
