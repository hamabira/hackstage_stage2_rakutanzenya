...
export function SubjectForm({
  ...
  const nextGradeItemId = useRef(
    gradeItems && gradeItems.length > 0 ? gradeItems.length : 1,
  );
  const [values, setValues] = useState<SubjectFormValues>(() => ({
    name: subject?.name ?? "",
    totalClassCount:
      subject?.totalClassCount == null ? "" : String(subject.totalClassCount),
    ...
    targetScore:
      subject?.targetScore == null ? "" : String(subject.targetScore),
    gradeItems: getInitialGradeItems(gradeItems),
  }));
  ...
  function updateValue<K extends Exclude<keyof SubjectFormValues, "gradeItems">>(
    key: K,
  ...
  function addGradeItem() {
    ...
    setValues((prev) => ({
      ...prev,
      gradeItems: [...prev.gradeItems, createGradeItem(id)],
    }));
  }
  ...
  function removeGradeItem(id: string) {
    setValues((prev) => ({
      ...prev,
      gradeItems: prev.gradeItems.filter((item) => item.id !== id),
    }));
  }
  ...
  function moveGradeItem(index: number, direction: "up" | "down") {
    ...
    setValues((prev) => {
      if (targetIndex < 0 || targetIndex >= prev.gradeItems.length) return prev;
      const next = [...prev.gradeItems];
      [next[index], next[targetIndex]] = [next[targetIndex], next[index]];
      return { ...prev, gradeItems: next };
    });
  }
    ...
    >
      ...
      <fieldset className="flex flex-col gap-4">
        ...
        <label
          ...
          htmlFor="total-class-count"
        >
          総授業回数
          <input
            aria-describedby={
              errors.totalClassCount ? getErrorId("totalClassCount") : undefined
            }
            ...
            onChange={(e) => updateValue("totalClassCount", e.target.value)}
            required
            step="1"
            type="number"
      ...
      <fieldset className="flex flex-col gap-4">
        ...
        <label
          className="flex flex-col gap-1 text-sm"
          htmlFor="attendance-required-rate"
        >
          必要出席率（%）
          <input
            aria-describedby={
              errors.attendanceRequiredRate
                ? getErrorId("attendanceRequiredRate")
                : undefined
            }
            ...
            onChange={(e) =>
              updateValue("attendanceRequiredRate", e.target.value)
            }
            step="0.01"
            type="number"
        ...
        <FieldError
          error={errors.attendanceRequiredRate}
          fieldName="attendanceRequiredRate"
        />

        <label
          className="flex flex-col gap-1 text-sm"
          htmlFor="attendance-max-absences"
        >
          最大欠席数
          <input
            aria-describedby={
              errors.attendanceMaxAbsences
                ? getErrorId("attendanceMaxAbsences")
                : undefined
            }
            ...
            onChange={(e) =>
              updateValue("attendanceMaxAbsences", e.target.value)
            }
            step="1"
            type="number"
        ...
        <FieldError
          error={errors.attendanceMaxAbsences}
          fieldName="attendanceMaxAbsences"
        />

        <label
          className="flex items-center gap-2 text-sm"
          htmlFor="attendance-affects-grade"
        >
          <input
            ...
            onChange={(e) =>
              updateValue("attendanceAffectsGrade", e.target.checked)
            }
            type="checkbox"
            value="true"
          />
      ...
      <fieldset className="flex flex-col gap-4">
        ...
        >
          ...
          <input
            ...
            onChange={(e) => updateValue("targetGradeLabel", e.target.value)}
            placeholder="例: 優（80点以上）"
            type="text"
        ...
        <label className="flex flex-col gap-1 text-sm" htmlFor="target-score">
          ...
          <input
            aria-describedby={
              errors.targetScore ? getErrorId("targetScore") : undefined
            }
            ...
            min="0"
            name="targetScore"
            onChange={(e) => updateValue("targetScore", e.target.value)}
            step="0.01"
            type="number"
      ...
      <fieldset className="flex flex-col gap-4">
        ...
        <p
          aria-live="polite"
          className={
            isWeightTotalValid ? "text-sm text-green-700" : "text-sm text-red-700"
          }
        >
          評価割合の合計: {weightTotal.toFixed(2)}%
          {isWeightTotalValid ? "（設定完了）" : "（100%にしてください）"}
        </p>
          ...
          {values.gradeItems.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                <input
                  name={`gradeItems[${index}][sortOrder]`}
                  type="hidden"
                  value={index}
                />
                ...
                <label
                  className="flex flex-col gap-1 text-sm"
                  htmlFor={`${itemPrefix}-name`}
                >
                  評価項目名
                  <input
                    aria-describedby={
                      errors[nameField] ? getErrorId(nameField) : undefined
                    }
                ...
                >
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateGradeItem(item.id, "category", e.target.value)
                    }
                    value={item.category}
                  >
                ...
                <label
                  className="flex flex-col gap-1 text-sm"
                  htmlFor={`${itemPrefix}-weight`}
                >
                  評価割合（%）
                  <input
                    aria-describedby={
                      errors[weightField] ? getErrorId(weightField) : undefined
                    }
                ...
                <label
                  className="flex flex-col gap-1 text-sm"
                  htmlFor={`${itemPrefix}-max-score`}
                >
                  満点
                  <input
                    aria-describedby={
                      errors[maxScoreField]
                        ? getErrorId(maxScoreField)
                        : undefined
                    }
                ...
                <FieldError
                  error={errors[maxScoreField]}
                  fieldName={maxScoreField}
                />
              </div>
            );
          })}
